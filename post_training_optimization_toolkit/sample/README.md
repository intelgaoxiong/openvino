# Post-training Optimization Tool samples {#pot_sample_README}
...
All available samples can be found in `<POT_DIR>/sample` folder, where `<POT_DIR>` is a directory where the Post-Training Optimization Tool is installed.
> **NOTE**: Standard path to the Post-Training Optimization Tool is `<INSTALL_DIR>/deployment_tools/tools/post_training_optimization_toolkit`, where 
