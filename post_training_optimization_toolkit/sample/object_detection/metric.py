...
class MAP(Metric):
    ...
    def update(self, prediction, annotation):
        """ Calculates and updates metric value
        :param prediction: model output
        :param annotation: annotations
        """
        annotation, shape_image = annotation[0]
        prediction = self.prepare_prediction(prediction, shape_image)

        per_class_results = []

        for label_id, label in enumerate(self.labels):
            detections, scores, dt_difficult = self.prepare_predictions_label(prediction, label)
            ground_truth, gt_difficult, iscrowd = self.prepare_annotations_label(annotation, label)
