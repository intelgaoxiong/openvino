# How to Run Examples {#pot_configs_examples_README}
...
In the instructions below, `<INSTALL_DIR>` is the directory where OpenVINO&trade; toolkit is
installed and `<POT_DIR>` is the Post-training Optimization Tool directory, which is `<INSTALL_DIR>/deployment_tools/tools/post_training_optimization_toolkit`.
`<EXAMPLE_DIR>` is the working directory where the example is executed.
...
For more information about the Benchmark Tool refer to its [documentation](@ref openvino_inference_engine_tools_benchmark_tool_README).
