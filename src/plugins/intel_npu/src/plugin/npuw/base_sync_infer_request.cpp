...
void ov::npuw::IBaseInferRequest::infer() {
    ...
    if (!m_npuw_model->m_using_pipeline_model) {
        ...
        m_profile[profile_tag(0u)].record([&]() {
            run_subrequest_for_success(0u, failover);
        });
        failover_happened |= failover;
        complete_subrequest(0u);
        if (m_npuw_model->m_acc_check) {
            ensure_subrequest_is_accurate(0u, failover);
            failover_happened |= failover;
        }
    }
...
void ov::npuw::IBaseInferRequest::bind_global_results(std::size_t idx, RqPtr request) {
    ...
    for (auto&& it : iodesc.global_results) {
        ...
        if (m_npuw_model->m_using_pipeline_model) {
            auto pipeline_out_port_idx{m_npuw_model->m_pipeline_global_outputs.find(static_cast<uint32_t>(result_idx))
            };
