...
KVCacheSlidingWindowManager::KVCacheSlidingWindowManager(uint32_t block_size,
    ...
    OPENVINO_ASSERT(block_size_ > 0 && window_size_ > 0,
                    "KVCacheSlidingWindowManager: block_size and window_size must be > 0, got block_size=",
                    block_size_,
                    " window_size=",
                    window_size_);
    ...
    OPENVINO_ASSERT(window_size_ % block_size_ == 0,
                    "KVCacheSlidingWindowManager: window_size (",
                    window_size_,
                    ") must be a multiple of block_size (",
                    block_size_,
                    "). Non-multiple window sizes are not yet supported.");
    num_slots_ = window_size_ / block_size_;

    // Detects which dim (2 or 3) of `base_shape` holds the sequence axis (whichever
    // equals block_size), mirroring KVCacheBlockManager's own detection logic.
    OPENVINO_ASSERT(base_shape.size() == 4 && (base_shape[2] == block_size_ || base_shape[3] == block_size_),
                    "KVCacheSlidingWindowManager: base_shape ",
                    base_shape,
                    " does not have block_size=",
                    block_size_,
                    " in sequence dimension (expected at dim 2 or 3)");
    seq_dim_ = (base_shape[2] == block_size_) ? 2u : 3u;

    ov::Shape window_shape = base_shape;
    window_shape[seq_dim_] = window_size_;
    window_ = ov::npuw::util::allocMem(elem_type, window_shape, device, plugin);

    LOG_INFO("KVCacheSlidingWindowManager initialized: block_size=" << block_size_ << ", window_size=" << window_size_
                                                                    << ", num_slots=" << num_slots_
                                                                    << ", seq_dim=" << seq_dim_
                                                                    << ", device=" << device);
}

ov::SoPtr<ov::ITensor> KVCacheSlidingWindowManager::get_slot_view(uint32_t slot_idx) const {
    OPENVINO_ASSERT(slot_idx < num_slots_,
                    "KVCacheSlidingWindowManager: slot index ",
                    slot_idx,
                    " out of range (num_slots=",
                    num_slots_,
                    ")");
    const uint32_t start = slot_idx * block_size_;
    const uint32_t end = start + block_size_;
    return ov::npuw::util::make_tensor_slice(window_, seq_dim_, start, end);
}

void KVCacheSlidingWindowManager::update(ov::SoPtr<ov::ITensor> src_new_kv,
                                         uint32_t src_kv_dim,
                                         uint32_t num_stored_tokens_before,
                                         uint32_t num_new_tokens) {
    ov::npuw::util::write_kv_slice_sliding(window_,
                                          src_new_kv,
                                          seq_dim_,
                                          src_kv_dim,
...
void KVCacheSlidingWindowManager::reset() {
    ov::npuw::util::fill_tensor_bytes(window_, 0u);
}
