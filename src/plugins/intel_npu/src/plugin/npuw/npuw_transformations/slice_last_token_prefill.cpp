...
SliceLastTokenPrefill::SliceLastTokenPrefill(uint32_t batch_dim) : m_batch_dim(batch_dim) {}
...
bool SliceLastTokenPrefill::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    //     get_ordered_ops() is a topological sort; transformer layers appear
    //     sequentially, so the last SDPA found is the last transformer layer.
    ...
    if (seq_len <= 1) {
        LOG_DEBUG("SliceLastTokenPrefill: seq_len <= 1, nothing to slice");
        return false;
    }
    ...
    auto find_downstream =
        [](const std::shared_ptr<ov::Node>& start, auto type_pred, int max_hops = 8) -> std::shared_ptr<ov::Node> {
        auto cur = start;
    ...
    auto o_proj = find_downstream(reshape_node, [](const std::shared_ptr<ov::Node>& n) {
    ...
    auto residual_add = find_downstream(o_proj, [](const std::shared_ptr<ov::Node>& n) {
    ...
    auto make_last_token_slice = [](const ov::Output<ov::Node>& src, int64_t dim, int64_t total) {
        auto start = ov::op::v0::Constant::create(ov::element::i64, {1}, {total - 1});
    ...
    // 2a. Slice Q: [B, H, N, D] → [B, H, 1, D].
    auto q_slice = make_last_token_slice(q_val, Q_SEQ_DIM, seq_len);
    last_sdpa->input(0).replace_source_output(q_slice->output(0));

    // 2b. Slice mask (if present): [B, 1, N, S] → [B, 1, 1, S].
    if (mask_seq_len > 0) {
        const auto mask_val = last_sdpa->input_value(3);
        auto mask_slice = make_last_token_slice(mask_val, 2, mask_seq_len);
        last_sdpa->input(3).replace_source_output(mask_slice->output(0));
    }

    // 2c. Update Reshape shape constant: N → 1 at seq dimension.
    if (shape_const->get_element_type() == ov::element::i32) {
        auto vals = shape_const->cast_vector<int32_t>();
        vals[reshape_seq_idx] = 1;
    ...
    // 2d. Slice shortcut: [B, N, hidden] → [B, 1, hidden].
    auto sc_slice = make_last_token_slice(sc_val, sc_seq_dim, sc_seq_len);
    residual_add->input(shortcut_port).replace_source_output(sc_slice->output(0));

    // 2e. Re-infer shapes; all other nodes (LayerNorm, FFN, final norm)
    //     propagate the new [B, 1, ...] shapes automatically.
