...
class Utilities:    
    ...
    EXCEL_OUTPUT_FILE = "singleImgComparison.xlsx"
    ...
    def GetBlobFiles(cls, arg_startingPath):
        blobs = Utilities.pathConcat(arg_startingPath, "/**/optimized/*.blob")
        blobFiles = glob.glob(blobs, recursive=True) 
        return blobFiles

    @classmethod
    def GetErrorFiles(cls, arg_startingPath):
        errors = Utilities.pathConcat(arg_startingPath, "/**/optimized/*.err")    
        errorFiles = glob.glob(errors, recursive=True)
        return errorFiles

    @classmethod
    def GetTotalNrOfNetworks(cls, arg_startingPath):
        xmlFiles = Utilities.pathConcat(arg_startingPath, "/**/optimized/*.xml") 
    ...
    def GetBenchmarkCommand(cls, arg_ovPath:str, arguments:str, device=False):
        ...
        if device == False:
            ...
            cmd = "cd " + arg_ovPath + "; source './bin/setupvars.sh' > /dev/null;" + " python3 /home/kmb/openvino_kmb_2021.4_extras/cross_check_wz.py " + arguments;
            # print(cmd)
        return cmd;
    ...
    class ExcelHeader(enum.Enum):
        ...
        CROSS_CHECK_MESSAGE = next(increment)    
    ...
    def buildArgumentString(cls,model=None, device="VPUX", nireq=8, config=None):        
        ...
        args +=" -d " + device
        args +=" -ref_d CPU"
        args +=" -ref_m " + model.replace("_MCM.blob",".xml")
        args +=" -i /home/kmb/openvino_kmb_2021.4_extras/models/pic/cat.jpg" 
        # args +=" -i " + model.replace(".blob",".jpg")
                ...
                if Utilities.isPartOfFailingNetworks(arg_blobFilePath):
                    ...
                    for nireq in paramsExtractor.getNireq():
                        try:                                                                
                            ...
                            print("xing test arguments: ",arguments)
                            # return
                        ...
                        if "Obtained values" in output.decode():
                            # print("xing test", output.decode())
                            excelEntry.update({Utilities.ExcelHeader.CROSS_CHECK_MESSAGE:data_clean(output.decode())}) 
...
def excelWriterGen(arg_fileName : str, arg_startLine : int):
    ...
    header = {
                ...
                Utilities.ExcelHeader.CROSS_CHECK_MESSAGE:"CrossCheckMessage"
             }
    ...
    ws.column_dimensions[get_column_letter(Utilities.ExcelHeader.CROSS_CHECK_MESSAGE.value)].width = 50
    ...
    try:
        while True:
            ...
            for idx, value in enumerate(Utilities.ExcelHeader):
                # print("xing test value :", idx+1,nextFreeRow,value,entry[value])
...
def main(ov_path=None):
    ...
    if not os.sys.argv[1].endswith(".json"):
        configFilePath = "./KpiScripts/SingleImgComparison/singleImgComparison_configs/singleImgComparison_config.json"
